...
function [Best_pos,Best_score,Convergence_curve]=SAO(repeat_num,take_num)
N=30;%SearchAgents_no
Max_iter=100; 

dim=5;

 lu=[1e-6,1e-6,0,700,7;
     1,2,1,900,18]; 

lb=lu(1,:);
ub=lu(2,:);
fobj=@func_repeat_train_1;
...
for i=1:size(X,1)
    Objective_values(1,i)=fobj(X(i,:),repeat_num,take_num);
    if i==1
        Best_pos=X(i,:);
