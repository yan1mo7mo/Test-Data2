function [mae] = op_ESN(param,data,inSize,outSize,train_r_step_cut,train_r_step_length,validate_r_step_length,len_washout)
inSize   = inSize;
outSize  = outSize;
data=data(train_r_step_cut+1:end,1:3);
trainLen = train_r_step_length;
predictionlen=validate_r_step_length;
initLen=len_washout;
...
% loss = norm(weighted_err);
% disp( ['loss = ', num2str( loss )] );

% mae=sum(abs(y_true-y_pred))/size(y_true,1)
% disp( ['mae = ', num2str( mae )] );

%rmse=sqrt(mean((y_pred - y_true).^2));
% diff = y_pred - y_true;              % 计算预测值与真实值之差
% rmse = sqrt(mean(diff(:).^2));       % 计算RMSE
mae=sum(abs(y_true-y_pred),'all')/ numel(y_true);
end